...
class ResumeDAO {
  ...
  public static ArrayList<Resume> readSample(
    ...
    for (String ratingOperator : ratingOperators) {
      String hql =
          "FROM Resume R WHERE R.rating "
              + ratingOperator
              + " :rating AND R.age BETWEEN :minAge AND :maxAge";
  ...
  /**
   * Запись резюме в базу данных. Производит операцию merge, изменяя уже существующие записи
   *
